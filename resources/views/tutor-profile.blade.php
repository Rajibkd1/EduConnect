                    ...
                    <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-8">
                        <div class="flex items-start space-x-6 mb-6">
                            <div class="relative">
                                ...
                                @else
                                    <div
                                        class="w-24 h-24 bg-gradient-to-br from-indigo-100 to-purple-100 rounded-full flex items-center justify-center border-4 border-indigo-200 shadow-lg">
                                ...
                                @endif
                                <div
                                    class="absolute -bottom-1 -right-1 w-6 h-6 bg-green-400 border-2 border-white rounded-full">
                                </div>
                            </div>
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-gray-500 mb-3">{{ $tutor->department }} • {{ $tutor->semester }}
                                    Semester</p>
                                ...
                                @if ($tutor->rating > 0)
                                    <div class="flex items-center mb-3">
                                        <div class="flex items-center mr-2">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <svg class="w-5 h-5 {{ $i <= $tutor->rating ? 'text-yellow-400' : 'text-gray-300' }}"
                                        ...
                                        <span class="text-gray-600 font-medium">{{ number_format($tutor->rating, 1) }}
                                            rating</span>
                                    </div>
                                @endif
                        ...
                        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                                @foreach ($tutor->subjects as $subject)
                                    <div
                                        class="bg-gradient-to-r from-indigo-100 to-purple-100 border border-indigo-200 rounded-lg p-3 text-center">
                    ...
                    <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
                        ...
                        <div class="space-y-3">
                            <a href="{{ route('direct-chat', $tutor->user->id) }}"
                                class="w-full px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl hover:from-indigo-700 hover:to-purple-700 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 inline-flex items-center justify-center">
                            ...
                            <a href="{{ route('session-requests.create', $tutor->id) }}"
                                class="w-full px-6 py-3 bg-white border-2 border-indigo-200 text-indigo-600 rounded-xl hover:bg-indigo-50 hover:border-indigo-300 transition-all duration-200 inline-flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M8 7V3a2 2 0 012-2h4a2 2 0 012 2v4m-6 0V6a2 2 0 012-2h4a2 2 0 012 2v1m-6 0h6m-6 0l-.5 9a2 2 0 002 2h3a2 2 0 002-2L16 7m-6 0V6a2 2 0 012-2h4a2 2 0 012 2v1" />
                                </svg>
                                Request Session
                            </a>

                            <button id="favorite-btn" data-tutor-id="{{ $tutor->id }}"
                                class="w-full px-6 py-3 bg-white border-2 border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all duration-200">
                                <svg id="favorite-icon" class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const tutorId = favoriteBtn.dataset.tutorId;

            // Check initial favorite status
            fetch(`/api/favorites/status/${tutorId}`)
                ...
                fetch('/api/favorites/toggle', {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            showNotification(data.error || 'Failed to update favorite status', 'error');
                        }
                    })
                    .catch(error => {
            ...
            function updateFavoriteButton(isFavorited) {
                if (isFavorited) {
                    favoriteBtn.classList.remove('bg-white', 'border-gray-200', 'text-gray-600', 'hover:bg-gray-50',
                        'hover:border-gray-300');
                    favoriteBtn.classList.add('bg-red-50', 'border-red-200', 'text-red-600', 'hover:bg-red-100',
                        'hover:border-red-300');
                    favoriteIcon.setAttribute('fill', 'currentColor');
                    favoriteText.textContent = 'Remove from Favorites';
                } else {
                    favoriteBtn.classList.remove('bg-red-50', 'border-red-200', 'text-red-600', 'hover:bg-red-100',
                        'hover:border-red-300');
                    favoriteBtn.classList.add('bg-white', 'border-gray-200', 'text-gray-600', 'hover:bg-gray-50',
                        'hover:border-gray-300');
